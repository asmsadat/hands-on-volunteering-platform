...
const EventModel = {
  getAllEvent: async() => {
    ...
    getByIdEvent: async (id) => {
    const result = await pool.query("SELECT * FROM events WHERE id = $1", [id]);
    return result.rows[0];
  },

    createEvent: async (id, title, description, date, time, location, category, createdBy, createdAt) => {
    const result = await pool.query(
      `INSERT INTO events (id, title, description, date, time, location, category, "createdBy", "createdAt")
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
      [id, title, description, date, time, location, category, createdBy, createdAt]
    );
    return result.rows[0];
  },

    updateEvent: async (id, title, description, date, time, location, category) => {
    const result = await pool.query(
